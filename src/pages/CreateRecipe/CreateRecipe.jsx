...
const CreateRecipe = () => {
	...
	const handlerSubmit = () => {
		axios
			.post(baseUrl + `recipe/${currentUser._id}`, data)
	...
	useEffect(() => {
		if (!diets.length) {
			axios.get(baseUrl + 'diet').then(res => setDiets(res.data.diets));
		}
	});
			...
			<div className={s.formContainer}>
				...
				<div className={s.formZone}>
					...
					<FormControl sx={{ m: 1, minWidth: 220 }} className={s.form2}>
						...
						<label>SUMMARY</label>
						<textarea name='summary' rows={10} onChange={handlerChange} />
						<label>INSTRUCTIONS</label>
						<textarea rows={10} name='instructions' onChange={handlerChange} />
						<TextField
							type='number'
